...
\begin{itemize}\setlength\itemsep{0.1em}
  \item CMS has run GPUs in the High Level Trigger since Run~3
  \item $\sim$\textbf{30\,\%} of HLT reconstruction is offloaded $\rightarrow$
        $\sim$\textbf{25\,\%} less HLT time
  \item ported: pixel local reco $\cdot$ pixel tracks and vertices $\cdot$
        calorimeter local reco
\end{itemize}
\vspace{0.4em}
...
\begin{itemize}\setlength\itemsep{0.1em}
  ...
  \item a serial data dependence is the one thing a GPU does badly
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
\begin{itemize}\setlength\itemsep{0.1em}
  ...
  \item \textbf{substructure} computed on demand
\end{itemize}
\smallnote{every one of these pays a host\,$\leftrightarrow$\,device round trip,
once per call}
\end{column}
\end{columns}
\end{frame}
...
\textbf{What it does}
\begin{itemize}\setlength\itemsep{0.1em}
  \item generalised-$k_t$ clustering --- \akt, $k_t$, Cambridge--Aachen
  \item runs on the GPU, \textbf{many jets or events per launch}
...
\begin{itemize}\setlength\itemsep{0.1em}
  \item Cacciari--Salam nearest-neighbour lemma: the smallest $d_{ij}$ in the
        \emph{whole event} is always some particle's \textbf{geometric} nearest
        neighbour
  \item so each particle only needs to track \textbf{its own} NN \ldots
  \item \ldots and one merge invalidates only $O(1)$ of them
  \item[$\Rightarrow$] \textbf{$O(N^2)$ instead of $O(N^3)$} --- the serial chain
        stays, but each step becomes cheap and parallel
\end{itemize}
\vspace{0.3em}
...
\smallnote{Written in \textbf{Triton}: compiles and autotunes for whatever GPU it
finds, so there is no separate CUDA build to maintain and results are
reproducible once the tuning is cached.}
\end{column}
\end{columns}
\end{frame}
