...
\begin{center}
\includegraphics[height=0.40\textheight]{fig/cms_hlt_gpu_timing.png}
\end{center}
\vspace{-0.5em}
\smallnote{CMS HLT per-event time, CPU-only (\textbf{690\,ms}) vs with GPU offload
(\textbf{398\,ms}) --- $\sim$\textbf{30\,\%} of the reco offloaded buys
...
\end{column}
\begin{column}{0.5\textwidth}
\textbf{Why that costs us now}\\[0.15em]
\smallnote{Inside a training loop the tensors are already on the GPU; each of
these sends them back to the host and returns.}
\begin{itemize}\setlength\itemsep{0.1em}
  \item \textbf{reclustering}
