...
\textbf{Model.} Particle Transformer [2202.03772] --- 8 encoder $+$ 2 class-attention
blocks, 8 heads, embed 128. \textbf{2.143\,M} params baseline,
\textbf{2.193\,M} with the feature block.
\smallskip

\textbf{Data.} JetClass, 10 classes, \textbf{20.05\,M} test jets
($\sim$2.0\,M/class), 128 particles/jet. All arms read \textbf{identical} shards;
features computed live.
\smallskip

\smallnote{Ranger, lr $10^{-3}$, batch 512, $10^{6}$ iterations, checkpoint every
20k. Uncertainties: bootstrap of the test set, 16--84th percentile.}
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{The feature arms}
\small
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \textbf{PLuM} --- 48 $k_t$-ordered Lund splittings, $[\ln k_t,
        \ln 1/\Delta R, \ln z]$, embedded and \textbf{concatenated} as extra
        tokens (176 vs 128)
  \item \textbf{subjet} --- $n$-body subjet mass, $p_T$ fraction, $n_{\rm const}$
  \item \textbf{CA5} --- per-particle C/A branch-point features
\end{itemize}
\end{block}
\begin{block}{Cost (wall clock per step)}
\small
\begin{tabular}{@{}lr@{}}
\toprule
...
\end{tabular}
\end{block}
\smallnote{\textbf{One seed per arm} --- see the caveat on the next slide.}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Does the tree help a tagger? --- result}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{Aggregate test accuracy} (20.05\,M jets)
\smallskip
...
\smallnote{AUC agrees to $\sim10^{-4}$ at every stage.}
\smallskip

\textbf{Rejection ratio @90\,\% eff.} (PLuM/baseline)
\smallskip
...
\toprule
 & 40k & 100k & 1M\\
\midrule
Hbb & 1.009 & \textbf{1.037} & 0.983\\
Hcc & 1.027 & 1.027 & 1.001\\
Hgg & 1.009 & 1.022 & 1.000\\
H4q & 1.037 & 1.012 & 1.005\\
Zqq & 1.011 & 1.002 & 1.000\\
Wqq & \textbf{1.036} & 1.001 & 0.999\\
Tbqq & \textbf{1.095} & 1.026 & 0.995\\
\bottomrule
\end{tabular}
\end{column}
\begin{column}{0.5\textwidth}
\begin{block}{What we see}
\small An \textbf{early gain in every class} --- Tbqq 1.095$\pm$0.016
($\sim$6$\sigma$), Wqq 1.036$\pm$0.004 ($\sim$9$\sigma$) --- that \textbf{decays
to nothing by 1\,M}. Wqq has the \emph{fewest} splittings yet gains as much as
anything: the effect is \textbf{not $b$-specific}.
\smallskip

At convergence \textbf{all three arms are null-or-worse}: plain training wins per
GPU-hour.
\end{block}
\begin{block}{What we do \emph{not} claim}
\small
\begin{itemize}\setlength{\itemsep}{1pt}
  \item \textbf{No mechanism.} $b$-specificity is excluded by the data; a
        substructure-complexity correlation was tested, not established.
  \item \textbf{One seed per arm.} A second PLuM seed gave a spread
        \emph{larger} than the effect --- the main weakness.
  \item We reproduce their Hbb \textbf{at 100k} (1.037), \textbf{not at
        convergence} (0.983).
\end{itemize}
\end{block}
\end{column}
...
\begin{itemize}\setlength{\itemsep}{3pt}
  ...
  \item H.~Qu, L.~Gouskos, \emph{ParticleNet: Jet Tagging via Particle Clouds},
        Phys.~Rev.~D \textbf{101} (2020) 056019, arXiv:1902.08570.
