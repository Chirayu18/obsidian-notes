...
$-1$ & \akt & the \textbf{hardest} pairs & --- & \textbf{finding jets} &
      \textbf{right} --- circular, rigid\\
$0$ & Cambridge--Aachen & the \textbf{closest} pair & \textbf{angle} &
      grooming, Lund plane & \textbf{middle} --- ragged edges\\
$+1$ & $k_t$ & the \textbf{softest} pair & \textbf{relative $p_T$} &
      exclusive subjets & \textbf{left} --- most irregular\\
\bottomrule
\end{tabular}
\end{center}
\vspace{-0.3em}
\smallnote{Only \akt{} gives jets whose area is insensitive to the soft
background --- which is why it is the one used to \emph{find} jets. The other
two are used on a jet that has \emph{already} been found.}
\end{frame}
...
\texttt{0} and \texttt{5} merge into \texttt{8} --- the node at the bottom right.
Seven columns, seven merges, and the tree is complete. \textbf{Everything
downstream --- grooming, subjets, Lund coordinates --- is a read of these rows},
...
\begin{center}
\makebox[\textwidth][c]{%
  \includegraphics[width=1.06\paperwidth]{fig/tree_gallery.png}}
\end{center}
\end{frame}
...
\begin{frame}{The benchmark --- two regimes \hfill \realC}
\begin{columns}[T]
...
\textbf{Large $N$, modest batch} --- the classic reconstruction task.
\end{block}
\smallnote{Same kernel, same API --- only the shape of the input tensor differs.
The regimes are timed separately because a jet and an event are different units.}
\end{column}
...
\smallnote{Grids: \textbf{3 algorithms $\times$ 5 radii $\times$ 5 multiplicity
bins} in each regime --- 150 jet points, 75 event points.}
\medskip

\textbf{Method} \smallnote{(identical in both)}
\begin{enumerate}\setlength\itemsep{0pt}
  \item extract constituents once, save;
  \item cluster on GPU, time it;
  \item cluster the \emph{same saved file} with FastJet;
  \item \textbf{check they agree} --- else the timing is meaningless.
\end{enumerate}
\smallskip

\smallnote{Speedups are quoted against the \textbf{vectorised} FastJet
interface --- the faster, fairer baseline.}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Time per unit --- both regimes \hfill \realC}
\begin{center}
\includegraphics[height=0.70\textheight]{fig/combined_regimes.png}
\end{center}
\vspace{-0.2em}
\smallnote{\textbf{Colour} = regime, \textbf{style} = implementation. The unit is
a \emph{jet} (blue) or an \emph{event} (orange) --- only the \textbf{ratio panel}
compares across them. \textbf{0.17--1.1\,$\mu$s/jet}, \textbf{4.9--40\,$\mu$s/event};
speedup peaks \textbf{129$\times$} / \textbf{65$\times$}.}
\end{frame}

\begin{frame}{How the speedup varies --- jet regime \hfill \realC}
\begin{center}
\includegraphics[width=\linewidth,trim=0 8 0 8,clip]{fig/atlas_speedup_algs.png}
\end{center}
\smallnote{All three algorithms, both samples, $R=1.0$. \textbf{54--143$\times$}
over vectorised FastJet across the 150-point grid (median \textbf{89$\times$}),
every point at \textbf{100\,\%} $n_{\rm jets}$ agreement. The speedup \textbf{peaks
near $N\approx45$}: below it the GPU is under-filled, above it the $O(N^2)$ work
per jet grows faster than FastJet's. The peak sits squarely in the
\textbf{boosted-object range}.}
\end{frame}
...
\begin{enumerate}
  ...
  \medskip
  \item \textbf{54--143$\times$ faster} than vectorised FastJet \textbf{in the jet
  regime} (one large-$R$ jet per timed unit, batched), peaking in the
  boosted-object range. Cost is independent of $R$ and of algorithm.
  \medskip
